...
void communicate(Matrix<double> &data, const Domain &domain) {
    ...
    if (domain.neighbours[0] != -1) {
       // std::cout << "In left " << rank << " \n";
        ...
        MPI_Sendrecv(send.data(), domain.size_y + 2, MPI_DOUBLE,rank,  domain.neighbours[0], recv.data(), domain.size_y + 2,
    ...
    if (domain.neighbours[1] != -1) {
        ...
        std::vector<double> recv(domain.size_y + 2);
        MPI_Sendrecv(send.data(), domain.size_y + 2, MPI_DOUBLE, rank,  domain.neighbours[1], recv.data(), domain.size_y + 2,
    ...
    if (domain.neighbours[2] != -1) {
        ...
        MPI_Sendrecv(send.data(), domain.size_x + 2, MPI_DOUBLE, domain.neighbours[2], 3, recv.data(), domain.size_x + 2,
    ...
    if (domain.neighbours[3] != -1) {
       // std::cout << "In bottom " << rank << " \n";
        ...
        std::vector<double> recv(domain.size_x + 2);
        MPI_Sendrecv(send.data(), domain.size_x + 2, MPI_DOUBLE, domain.neighbours[3], 4, recv.data(), domain.size_x + 2,
