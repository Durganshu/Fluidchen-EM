...
class PressureSolver {
  public:
     ...
     * @param[in] boundary to be used
     */
    virtual double solve_potential(Fields &field, Grid &grid,
                                   const std::vector<std::unique_ptr<PotentialBoundary>> &boundaries) = 0;
};
...
class SOR : public PressureSolver {
  public:
    ...
    /**
     * @brief Solve the potential equation on given field, grid and boundary
     *
     * @param[in] field to be used
     * @param[in] grid to be used
     * @param[in] boundary to be used
     */
    virtual double solve_potential(Fields &field, Grid &grid, const std::vector<std::unique_ptr<PotentialBoundary>> &boundaries);
